...
typedef struct {
    ...
    CThreadSafeList<TRANS_MSG>  list_trans; /* different thread trans msg each other. */
} LIBEVENT_THREAD;
...
private:
    ...
    /* parse the data */
    static CONN* FindThreadFromId(int id);
    static void TransMsg(CONN* conn, TRANS_MSG tm);
    static void UsingConn(CONN* conn);
    static void UnusingConn(CONN* conn);
...
private:
    ...
    static std::vector<CONN*> vec_using_conn_;
